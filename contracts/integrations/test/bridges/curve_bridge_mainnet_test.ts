    ...
    describe('bridgeTransferFrom()', () => {
        describe('exchange_underlying()', () => {
            ...
            it('succeeds exchanges USDC for DAI', async () => {
                ...
                // Fund the Bridge
                const usdc = new ERC20TokenContract(USDC_ADDRESS, env.provider, { ...env.txDefaults, from: USDC_WALLET });
                await usdc
                    .transfer(testContract.address, toBaseUnitAmount(1, 6))
                    .awaitTransactionSuccessAsync({}, { shouldValidate: false });
                // Exchange via Curve
                await testContract
                    .bridgeTransferFrom(DAI_ADDRESS, constants.NULL_ADDRESS, RECEIVER, constants.ZERO_AMOUNT, bridgeData)
                    .awaitTransactionSuccessAsync({}, { shouldValidate: false });
            });
        });
        ...
        describe('exchange()', () => {
            ...
            it('succeeds exchanges WBTC for renBTC', async () => {
                ...
                // Fund the Bridge
                const wbtc = new ERC20TokenContract(WBTC_ADDRESS, env.provider, { ...env.txDefaults, from: WBTC_WALLET });
                await wbtc
